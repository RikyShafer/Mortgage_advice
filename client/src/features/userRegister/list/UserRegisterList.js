...
const UserRegisterList = () => {
    ...
    return (
        <div>
            <div className="register-list-title-user">תצוגה איזה אנשים נרשמו לאתר</div>
            <div className="register-list">
                {usersData.map((user) => (
                    <div className="register-list-Map" key={user._id}>
                        ...
                        <p>דוא"ל - <a href={`https://mail.google.com/mail/?view=cm&fs=1&tf=1&to=${encodeURIComponent(user.email)}`} className="google-link" target="_blank" rel="noopener noreferrer">{user.email}</a></p>
                        <p>טלפון - <a href={`tel:${user.phone}`} className="google-link" rel="noopener noreferrer">{user.phone}</a></p>
